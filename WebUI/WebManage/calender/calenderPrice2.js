...
function changecal(action, year, month, flag) {
    if (flag == 1) {
      ...
      case "nextmonth":
            ...
            strcal = "<span onmouseover=\"this.className='arrow_over'\" onmouseout=\"this.className='arrow_out'\" class='arrow_out'  onclick='new_calender(" + year + "," + month + ")' title='下一个月' style='cursor:hand;'>></span>";
            break;
      case "premonth":
            if(month==0)
            {
            ...
            strcal = "<span onmouseover=\"this.className='arrow_over'\" onmouseout=\"this.className='arrow_out'\" class='arrow_out' onclick='new_calender(" + year + "," + month + ")' title='上一个月' style='cursor:hand;'><</span>";
            break;
      case "nextyear":
            year = year*1 + 1;
            month = month*1 + 1;
...
//===========输出日历===========
      ...
      for(count=1;count<=totalcounts;count++)
      {
            ...
            if((count-firstspace)<=monthdays)
            {
				  ...
				  if (checkdate(Mdate, time1)) {
				      str += "<td class=\"normal\" id=\"" + time1 + "\"><a onclick=\"show1(event,'" + time1 + "')\"><em style='color:red;'>" + (count - firstspace) + "</em></a></td>";
				  }
				  else {

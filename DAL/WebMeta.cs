...
namespace ClassLibrary.DAL
{
    ...
    public class WebMeta
    {
        ...
        /// 表名
        /// </summary>
		private static readonly string table = "WebMeta";
		/// <summary>
        /// 主键列
        /// </summary>
        ...
        public int Update(ClassLibrary.Model.WebMeta model)
        {
            ...
            strSql.Append("QQ=@QQ,");
			strSql.Append("OnlineService=@OnlineService,");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@OnlineService", SqlDbType.NVarChar, 1000),
					new SqlParameter("@CreatedTime", SqlDbType.DateTime, 8)};
            ...
            parameters[6].Value = model.@OnlineService;
        ...
        public ClassLibrary.Model.WebMeta GetModel(int ID)
        {
            ...
            SqlParameter[] parameters = {
                    new SqlParameter("@" + pk, SqlDbType.Int ,4)};
            parameters[0].Value = ID;
            ClassLibrary.Model.WebMeta model = new ClassLibrary.Model.WebMeta();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["Keyword"] != DBNull.Value)
				{
					model.Keyword = Convert.ToString(dt.Rows[0]["Keyword"]);
				}
				...
				if (dt.Rows[0]["OnlineService"] != DBNull.Value)
                {
                    model.OnlineService = Convert.ToString(dt.Rows[0]["OnlineService"]);
                }
        ...
        public DataSet GetPageData(int pageSize, int pageNum, string strWhere, string orderBy)
        {
            return SQLHelper.GetPageData(pageSize, pageNum
                    , "ID,Title,Keyword,Description,Telphone,CreatedTime"
                    , table

...
namespace ClassLibrary.BLL
{
    ...
    public class Advertise
    {
        ...
        /// 数据操作实例
        /// </summary>
        private readonly ClassLibrary.DAL.Advertise dal = new ClassLibrary.DAL.Advertise();
        //private readonly IAdvertise dal = DataAccess.CreateAdvertise();
        /// <summary>
        /// 排序
        /// </summary>
        ...
        /// </summary>
        /// <param name="strWhere">条件</param>
        /// <returns>Advertise集合</returns>
        public List<ClassLibrary.Model.Advertise> GetModelList(string strWhere)
        {
            DataTable dt = GetData(strWhere);
            List<ClassLibrary.Model.Advertise> modelList = new List<ClassLibrary.Model.Advertise>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                ClassLibrary.Model.Advertise model;
                for (int n = 0; n < rowsCount; n++)
                {
                    ...
                    if (dt.Rows[n]["positionID"] != DBNull.Value)
					{
						model.positionID = Convert.ToInt32(dt.Rows[n]["positionID"]);
					}

...
namespace ClassLibrary.BLL
{
    ...
    public class Links
    {
        ...
        /// 数据操作实例
        /// </summary>
        private readonly ClassLibrary.DAL.Links dal = new ClassLibrary.DAL.Links();
        ...
        public List<ClassLibrary.Model.Links> GetModelList(string strWhere)
        {
            DataTable dt = GetData(strWhere);
            List<ClassLibrary.Model.Links> modelList = new List<ClassLibrary.Model.Links>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                ClassLibrary.Model.Links model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new ClassLibrary.Model.Links();
                    ...
                    if (dt.Rows[n]["LinkClass"] != DBNull.Value)
                    {
                        model.LinkClass = Convert.ToInt32(dt.Rows[n]["LinkClass"]);
                    }
                    modelList.Add(model);
                }
            }
        ...
        /// <param name="strWhere">条件</param>
        /// <returns>Links集合</returns>
        public List<ClassLibrary.Model.Links> GetModelList(int top, string strWhere)
        {

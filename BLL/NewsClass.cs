...
namespace ClassLibrary.BLL
{
    ...
    public class NewsClass
    {
        ...
        /// 数据操作实例
        /// </summary>
        private readonly ClassLibrary.DAL.NewsClass dal = new ClassLibrary.DAL.NewsClass();
        ...
        /// </summary>
        /// <param name="strWhere">条件</param>
        /// <returns>NewsClass集合</returns>
        public List<ClassLibrary.Model.NewsClass> GetModelList(string strWhere)
        {
            DataTable dt = GetData(strWhere);
            List<ClassLibrary.Model.NewsClass> modelList = new List<ClassLibrary.Model.NewsClass>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                ClassLibrary.Model.NewsClass model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new ClassLibrary.Model.NewsClass();

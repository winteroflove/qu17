...
namespace ClassLibrary.BLL
{
    ...
    public class OrderDetail
    {
        ...
        /// 数据操作实例
        /// </summary>
        private readonly ClassLibrary.DAL.OrderDetail dal = new ClassLibrary.DAL.OrderDetail();
        ...
        /// </summary>
        /// <param name="strWhere">条件</param>
        /// <returns>OrderDetail集合</returns>
        public List<ClassLibrary.Model.OrderDetail> GetModelList(string strWhere)
        {
            DataTable dt = GetData(strWhere);
            List<ClassLibrary.Model.OrderDetail> modelList = new List<ClassLibrary.Model.OrderDetail>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                ClassLibrary.Model.OrderDetail model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new ClassLibrary.Model.OrderDetail();
                    ...
                    if (dt.Rows[n]["Number"] != DBNull.Value)
					{
                        model.Number = Convert.ToString(dt.Rows[n]["Number"]);
					}
                    if (dt.Rows[n]["RouteName"] != DBNull.Value)
					{
						model.RouteName = Convert.ToString(dt.Rows[n]["RouteName"]);
					}
                    if (dt.Rows[n]["RoutePrice"] != DBNull.Value)
					{
						model.RoutePrice = Convert.ToDecimal(dt.Rows[n]["RoutePrice"]);
					}
                    if (dt.Rows[n]["RouteTime"] != DBNull.Value)
					{
						model.RouteTime = Convert.ToString(dt.Rows[n]["RouteTime"]);
					}
                    if (dt.Rows[n]["StartTime"] != DBNull.Value)
					{
						model.StartTime = Convert.ToString(dt.Rows[n]["StartTime"]);
					}

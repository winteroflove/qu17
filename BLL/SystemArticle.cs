...
namespace ClassLibrary.BLL
{
    ...
    public class SystemArticle
    {
        ...
        /// 数据操作实例
        /// </summary>
        private readonly ClassLibrary.DAL.SystemArticle dal = new ClassLibrary.DAL.SystemArticle();
        //private readonly ISystemArticle dal = DataAccess.CreateSystemArticle();
        /// <summary>
        /// 排序
        /// </summary>
        ...
        /// </summary>
        /// <param name="strWhere">条件</param>
        /// <returns>SystemArticle集合</returns>
        public List<ClassLibrary.Model.SystemArticle> GetModelList(string strWhere)
        {
            DataTable dt = GetData(strWhere);
            List<ClassLibrary.Model.SystemArticle> modelList = new List<ClassLibrary.Model.SystemArticle>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                ClassLibrary.Model.SystemArticle model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new ClassLibrary.Model.SystemArticle();
                    if (dt.Rows[n]["ID"] != DBNull.Value)
					{
						model.ID = Convert.ToInt32(dt.Rows[n]["ID"]);
					}
                    if (dt.Rows[n]["classID"] != DBNull.Value)
					{
						model.classID = Convert.ToInt32(dt.Rows[n]["classID"]);
					}
